...
def perform_copy_job_through_quickset(scan_emulation, cdm, udw, spice, net, job, quickset_name, settings: dict, scanner_source="ADF"):
    ...
    try:
        ...
        href = CdmEndpoints.JOB_TICKET_ENDPOINT+ "/" + jobticketid
...
def delete_custom_shortcut(cdm, net, shortcut_id):
    '''
    Delete the shortcut
    '''
    cdm_shortcut_service = CDMShortcuts(cdm, net)
...
def delete_all_shortcuts(cdm, net):
    '''
    Delete the shortcut
    '''
    cdm_shortcut_service = CDMShortcuts(cdm, net)
    cdm_shortcut_service.delete_all_shortcuts()

def create_custom_shortcut(cdm, net, shortcut_name, shortcut_id):
    '''
    ...
    cdm_shortcut_service.create_custom_shortcut(shortcut_name, shortcut_id, "scan", [
                                   "print"], "open", "true", False, cdm_shortcut_service.JobTicketType.COPY)

def validate_custom_shortcut(cdm, net, shortcut_name):
    '''
    Validate the custom shortcut
    '''
    cdm_shortcut_service = CDMShortcuts(cdm, net)
    shortcut_id = cdm_shortcut_service.get_shortcut_id(shortcut_name)
    logging.info("shortcut_id %s", shortcut_id)
    return shortcut_id
    # find_quick_set_by_title(quickset)
    #     assert qs_index != -1, f"Failed to find corresponding quickset {quickset}"

    # cdm_shortcut_service.validate_custom_shortcut(shortcut_id, shortcut_name, source, destination, open, is_default, is_hidden)
...
def modify_copy_job_ticket(cdm, ticket_body, jobticketId, settings: dict):
    ...
    if "outputMediaSize" in settings.keys():
        ...
        if "inputMediaSize" in settings.keys():
            if ticket_body['dest']['print']['mediaSize'] == 'any':
                ticket_body['dest']['print']['mediaSize'] = settings["inputMediaSize"]
    ...
    uri = CdmEndpoints.JOB_TICKET_ENDPOINT + "/" + jobticketId
